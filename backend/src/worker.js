...
const processSingleReview = async (review, profile) => {
  try {
    ...
    console.error(`Error processing review ${review.google_review_id}:`, error);
  }
};
...
const processReviews = async () => {
  ...
  try {
    ...
    for (const profile of profiles) {
      ...
      // Process reviews in chunks of 5 to avoid overwhelming the external APIs and DB
      const chunkSize = 5;
      for (let i = 0; i < newReviews.length; i += chunkSize) {
        const chunk = newReviews.slice(i, i + chunkSize);
        await Promise.all(chunk.map(review => processSingleReview(review, profile)));
      }
    }
